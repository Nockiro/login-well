 ...
 <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script> 
  ...
  <script type="text/javascript">
	...
	function drawBackgroundColor() {
          ...
          data.addRows([		
		  <?php 
			$query = "SELECT `session_begin`, `duration`, `pid` FROM visits WHERE `uid` = $uid";
			...
			foreach ($array as $ind => $data){ //every site we have data from is iterated..
				foreach ($data as $key => $val){
					if ($key == "session_begin"){ //and the date stored as x-variable...
						...
						foreach (range(1,$pIDPos) as $num) { // zeros are added in front of the statement to put $var in the right place
							if ($num > 0){
								$addition = "0,".$addition;
							}
						}
